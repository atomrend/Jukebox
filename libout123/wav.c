...
#include "libout123/out123_int.h"
#include "libout123/wav.h"
...
static void* wavhead_new(void const *template_, size_t size)
{
	void *header = malloc(size);
	if(header)
		memcpy(header, template_, size);
	return header;
}
...
static int open_file(struct wavdata *wdat, char *filename)
{
	...
	int setuidResult = setuid(getuid());
#endif
...
int wav_open(out123_handle *ao)
{
	...
	{
		if(!(inthead = wavhead_new(&riff_template, sizeof(riff_template))))
		{
			ao->errcode = OUT123_DOOM;
			goto wav_open_bad;
		}
