...
static int
find_synth(func_synth synth,  const func_synth synths[r_limit][f_limit])
{
    enum synth_resample ri;
    enum synth_format fi;
    int rIndex = 0;
    int rLast = (int) r_limit;
    int fIndex;
    int fLast = (int) f_limit;

    while (rIndex < rLast) {
        ri = (enum synth_resample) rIndex;
        fIndex = 0;
        while (fIndex < fLast) {
            fi = (enum synth_format) fIndex;
...
#		endif
	...
	if(type != nodec)
	{
		...
		fr->cpu_opts.class_ = decclass(type);

		debug3("determined active decoder type %i (%s) of class %i", type, decname[type], fr->cpu_opts.class_);
		return MPG123_OK;
	}
	else
	{
...
#		endif
	...
	fr->cpu_opts.class_ = decclass(fr->cpu_opts.type);
...
enum optdec dectype(const char* decoder)
{
    ...
    while (index < 20) {
        dt = (enum optdec) index;
        if(strcasecmp(decoder, decname[dt]) == 0) {
            return dt;
        }
        ++index;
    }
