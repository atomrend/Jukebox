...
int attribute_align_arg mpg123_grow_string(mpg123_string* sb, size_t new_)
{
	if(!sb)
		return 0;
	if(sb->size < new_) return mpg123_resize_string(sb, new_);
	else return 1;
}

int attribute_align_arg mpg123_resize_string(mpg123_string* sb, size_t new_)
{
	if(!sb)
		return 0;
	debug3("resizing string pointer %p from %lu to %lu", (void*) sb->p, (unsigned long)sb->size, (unsigned long)new_);
	if(new_ == 0)
	{
	...
	if(sb->size != new_)
	{
		char* t;
		debug("really!");
		t = (char*) safe_realloc(sb->p, new_*sizeof(char));
