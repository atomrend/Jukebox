	...
	*newheadp = 0;
	...
	if( !(fr->p.flags & MPG123_NO_RESYNC) )
	{
		...
		do /* ... shift the header with additional single bytes until be found something that could be a header. */
		{
			...
			if(VERBOSE3) debug3("resync try %li at %"OFF_P", got newhead 0x%08lx", try_, (off_p)fr->rd->tell(fr),  newhead);
		...
		*newheadp = newhead;
		...
		{
			if(NOQUIET)
			error1("Giving up resync after %li bytes - your stream is not nice... (maybe increasing resync limit could help).", try_);
