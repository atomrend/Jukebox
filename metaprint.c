...
,	const mpg123_string *tag, enum tagcode fi, int long_mode )
{
	...
	if(long_mode)
		fprintf(out, "\t");
	snprintf(fmt, sizeof(fmt)-1, "%%s:%%-%lus%%s\n", 1+namelen[0]-strlen(name[fi]));
...
,	enum tagcode f0, enum tagcode f1 /* field indices for column 0 and 1 */
	  ...
	  && len[f0] <= (size_t)climit[0] && len[f1] <= (size_t)climit[1] )
	{
		...
		snprintf( cfmt, sizeof(cfmt)-1, "%%s:%%-%lus%%-%ds  %%s:%%-%lus%%-%ds\n"
...
void print_id3_tag(mpg123_handle *mh, int long_id3, FILE *out)
{
    ...
    len[0] = 0;
    mpg123_init_string(&tag[0]);
    len[1] = 0;
    mpg123_init_string(&tag[1]);
    len[2] = 0;
    mpg123_init_string(&tag[2]);
    len[3] = 0;
    mpg123_init_string(&tag[3]);
    len[4] = 0;
    mpg123_init_string(&tag[4]);
    len[5] = 0;
    mpg123_init_string(&tag[5]);
    len[6] = 0;
    mpg123_init_string(&tag[6]);
