...
int main(int sys_argc, char ** sys_argv)
{
	...
	mpg123_delete_pars(mp); /* Don't need the parameters anymore ,
        they're in the handle now. */
...
/* Cygwin --realtime seems to fail when accessing network, using win32 set 
    priority instead */
...
/* make sure not Cygwin, it doesn't need it */
...
#if !defined(WIN32) && !defined(GENERIC)
	/* Remote mode is special... but normal console and terminal-controlled 
        operation needs to catch the SIGINT.
	   For one it serves for track skip when not in terminal control mode.
	   The more important use being a graceful exit, including telling the 
        buffer process what's going on. */
	if(!param.remote)
...
#if 0 /* Disables this block to be replaced with jukebox code. */
	while ((fname = get_next_file()))
	{
		...
		if(param.delay > 0)
		{
			...
			if(param.verbose > 2) fprintf(stderr, "Note: pausing %i seconds "
                "before next track.\n", param.delay);
#ifdef WIN32
		...
		/*
			...
			Rationale: Because of libmpg123 sample accuracy, this could cause 
            an unnecessary backwards seek, that even may fail on non-seekable streams.
		...
		*/
		...
		if(framenum < 0)
		{
			...
			{
				fprintf(stderr, "%s", "So, you have trouble getting an "
                    "output format... this is the matrix of currently "
                    "possible formats:\n");
				print_capabilities(ao, mh);
				fprintf(stderr, "%s", "Somehow the input data and your "
                    "choices don't allow one of these.\n");
			}
			mpg123_close(mh);
			continue;
		} // if(framenum < 0)
...
#endif /* end of #if disabling this block from the original code. */
